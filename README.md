...
<details><summary>Развернуть Задача №2: докеризация приложения (необязательная)</summary>
...
* Запуск построения образа: `docker image build -t db-api:1.0 .` (_должен быть Dockerfile в текущей директории_)
* Первый запуск контейнера: `docker-compose up --build` (_должен быть docker-compose.yml в текущей директории_)
* Последующие запуски: `docker-compose up` (_должен быть docker-compose.yml в текущей директории_)
